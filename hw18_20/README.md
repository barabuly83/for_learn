...
Для локальной разработки используются Firebase эмулятор (Storage). **Это учебный проект - данные не сохраняются в production.**

### Запуск эмуляторов
```bash
# Запуск Storage эмуляторов
firebase emulators:start --only storage

# Импорт тестовых данных
firebase emulators:start --only storage --import=./storage_data

# Экспорт данных для тестирования
firebase emulators:start --only storage --export-on-exit=./storage_data
```
