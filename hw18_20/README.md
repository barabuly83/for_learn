...
├── data/                    # Слой данных
│   ├── datasources/         # Источники данных (Firebase, Local)
...
## 🎓 Учебный проект - Firebase эмуляторы

Для локальной разработки используются Firebase эмуляторы (Firestore + Storage). **Это учебный проект - данные не сохраняются в production.**

### Запуск эмуляторов
```bash
# Запуск Firestore и Storage эмуляторов
firebase emulators:start --only firestore,storage

# Импорт тестовых данных
firebase emulators:start --only firestore,storage --import=./storage_data

# Экспорт данных для тестирования
firebase emulators:start --only firestore,storage --export-on-exit=./storage_data
```
